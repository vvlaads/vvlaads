...
public class UpdateCommand implements Command, Serializable {
    ...
    public String execute(Argument argument, Worker newWorker) {
        ...
        if (!getCollectionManager().hasID(id)) {
            ...
            newWorker.setId(id);
            try {
                if (getCommandManager().getDatabaseManager().updateWorker(newWorker, id, getCommandManager().getClientData())) {
                    getCollectionManager().getEntrySet().stream()
                            .filter(worker -> worker.getValue().getId() == id).
                            forEach(worker -> getCollectionManager().put(worker.getKey(), newWorker));
                    return ("Объект успешно обновлен");
                }
                return ("Нельзя изменить этот объект");
