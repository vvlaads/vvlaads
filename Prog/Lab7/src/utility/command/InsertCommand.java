    ...
    public String execute(Argument argument, Worker worker) {
        if (!Calculate.stringIsInteger(argument.getName())) {
            ...
            if (getCollectionManager().hasKey(key)) {
                ...
                try {
                    if (getCommandManager().getDatabaseManager().insertWorker(worker, getCommandManager().getClientData())) {
                        worker.setId(getCommandManager().getDatabaseManager().getLastWorker());
                        getCollectionManager().put(key, worker);
                        return ("Объект успешно добавлен");
                    } else {
                        return ("Не удалось добавить в Базу Данных");
                    }
