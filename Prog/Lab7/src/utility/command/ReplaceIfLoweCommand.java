    ...
    public String execute(Argument argument, Worker worker) {
        if (!Calculate.stringIsInteger(argument.getName())) {
            ...
            if (!getCollectionManager().hasKey(key)) {
                ...
                try {
                    if (worker.compareTo(getCollectionManager().get(key)) < 0) {
                        Worker oldWorker = getCollectionManager().get(key);
                        if (getCommandManager().getDatabaseManager().updateWorker(worker, oldWorker.getId(), getCommandManager().getClientData())) {
                            getCollectionManager().put(key, worker);
                        }
